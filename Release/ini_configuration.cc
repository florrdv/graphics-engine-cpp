...
namespace ini {
    ...
    ParseException& ParseException::operator=(
        const ParseException& original) throw() {
        std::exception::operator=(original);
        return *this;
    }

    UnexpectedCharacter::UnexpectedCharacter(
        const std::istream::int_type character_init,
    ...
    UnexpectedCharacter::UnexpectedCharacter(
        const UnexpectedCharacter& original) throw()
        : ParseException(),
    ...
    UnexpectedCharacter& UnexpectedCharacter::operator=(
        const UnexpectedCharacter& original) throw() {
        std::exception::operator=(original);
    ...
    DuplicateSection::DuplicateSection(const std::string& name_init) throw()
        ...
        message =
            "section named \'" + name + "\' encoutered twice in configuration file";
    }
    ...
    DuplicateSection& DuplicateSection::operator=(
        const DuplicateSection& original) throw() {
        std::exception::operator=(original);
    ...
    const char* DuplicateSection::what() const throw() { return message.c_str(); }
    ...
    DuplicateEntry::DuplicateEntry(const std::string& section_init,
        ...
        message = "entry named \'" + key + "\' ecountered twice in section \'" +
            section + "\'";
    }
    ...
    DuplicateEntry& DuplicateEntry::operator=(
        const DuplicateEntry& original) throw() {
        std::exception::operator=(original);
    ...
    NonexistentEntry::NonexistentEntry(const std::string& section_name_init,
        ...
        message("") {
        message = "entry with name \'" + entry_name +
            "\' does not exist in section \'" + section_name + "\'";
    }
    ...
    NonexistentEntry& NonexistentEntry::operator=(
        const NonexistentEntry& original) throw() {
        std::exception::operator=(original);
        message = original.message;
        return *this;
    }
    ...
    IncompatibleConversion::IncompatibleConversion(
        const std::string& section_name_init, const std::string& entry_name_init,
        ...
        message("") {
        message = "cannot convert value of \'" + entry_name + "\' in section \'" +
            section_name + "\' to " + type_name;
    }
    ...
    IncompatibleConversion& IncompatibleConversion::operator=(
        const IncompatibleConversion& original) throw() {
        std::exception::operator=(original);
    ...
    namespace {
        ...
        bool ValueBase::exists() const { return true; }
        ...
        IntValue::IntValue(const int value_init) : ValueBase(), value(value_init) {
            // Does nothing...
        }
        ...
        BoolValue::BoolValue(const bool value_init) : ValueBase(), value(value_init) {
            // Does nothing...
        }
        ...
        void TupleValue::delete_element(Value* const element) { delete element; }
        ...
        bool TupleValue::as_int_tuple_if_exists(const std::string& section_name,
            ...
            for (ConstElementIter i = first_element; i != last_element; ++i) {
                int element_value;
                const bool exists =
                    (*i)->as_int_if_exists(section_name, entry_name, element_value);
                assert(exists);
        ...
        bool TupleValue::as_double_tuple_if_exists(const std::string& section_name,
            ...
            for (ConstElementIter i = first_element; i != last_element; ++i) {
                ...
                const bool exists =
                    (*i)->as_double_if_exists(section_name, entry_name, element_value);
                assert(exists);
                ret_val.push_back(element_value);
            }
        ...
        void delete_value(Value* const value) { delete value; }

        void delete_entry(const ValueMap::value_type& entry) { delete entry.second; }
        ...
        bool is_hspace(std::istream::int_type chr) { return chr == '\t' || chr == ' '; }
        ...
        void assert_chars(std::istream& input_stream, const char* const chars) {
        ...
        ValueMap read_entries(const std::string& name, std::istream& input_stream) {
            ValueMap values;
    ...
    const std::string& Entry::get_section_name() const { return section_name; }

    const std::string& Entry::get_entry_name() const { return entry_name; }

    bool Entry::exists() const { return value_ptr->exists(); }
    ...
    bool Entry::as_double_tuple_if_exists(DoubleTuple& ret_val) const {
        return value_ptr->as_double_tuple_if_exists(section_name, entry_name,
            ret_val);
    }
    ...
    DoubleTuple Entry::as_double_tuple_or_default(
        const DoubleTuple& def_val) const {
        DoubleTuple value;
    ...
    Entry::operator int() const { return as_int_or_die(); }

    Entry::operator double() const { return as_double_or_die(); }

    Entry::operator std::string() const { return as_string_or_die(); }

    Entry::operator bool() const { return as_bool_or_die(); }

    Entry::operator IntTuple() const { return as_int_tuple_or_die(); }

    Entry::operator DoubleTuple() const { return as_double_tuple_or_die(); }
    ...
    Configuration::Configuration(std::istream& input_stream) : sections() {
        parse(input_stream);
    }
